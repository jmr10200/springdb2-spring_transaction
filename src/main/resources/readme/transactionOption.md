...
#### 다양한 전파 옵션
기본 값 : REQUIRED<br>
-> 대부분 REQUIRED 를 사용하므로 나머지 옵션들은 필요할 때 찾아 사용하자.<br>

* REQUIRED
  * 가장 많이 사용하는 기본 설정
  * 기존 트랜젝션이 없으면 생성, 있으면 참여
  * 트랜젝션이 필수 라는 의미로 이해하자 (필수이므로 없으면 생성, 있으면 참여)
    * 기존 트랜젝션 없음 : 새 트랜젝션 생성
    * 기존 트랜젝션 있음 : 기존 트랜젝션 참여
* REQUIRED_NEW
  * 항상 새로운 트랜젝션을 생성
    * 기존 트랜젝션 없음 : 새 트랜젝션 생성
    * 기존 트랜젝션 있음 : 새 트랜젝션 생성
* SUPPORT
  * 트랜젝션 지원
    * 기존 트랜젝션 없음 : 트랜젝션 없이 진행
    * 기존 트랜젝션 있음 : 기존 트랜젝션에 참여
* NOT_SUPPORT
  * 트랜젝션을 지원하지 않음
    * 기존 트랜젝션 없음 : 트랜젝션 없이 진행
    * 기존 트랜젝션 있음 : 트랜젝션 없이 진행 (기존 트랜젝션은 보류함)
* MANDATORY
  * 의무사항. 트랜젝션이 반드시 있어야함
  * 트랜젝션이 없으면 예외 발생
    * 기존 트랜젝션 없음 : IllegalTransactionStateException 발생
    * 기존 트랜젝션 있음 : 기존 트랜젝션에 참여
* NEVER
  * 트랜젝션을 사용하지 않음
  * 기존 트랜젝션이 있으면 예외가 발생
    * 기존 트랜젝션 없음 : 트랜젝션 없이 진행
    * 기존 트랜젝션 있음 : IllegalTransactionStateException 발생
* NESTED
  * 기존 트랜젝션 없음 : 새 트랜젝션 생성
  * 기존 트랜젝션 있음 : 중첩 트랜젝션 생성
    * 중첩 트랜젝션은 외부 트랜젝션의 영향을 받지만, 중첩 트랜젝션은 외부에 영향 주지 않음
    * 중첩 트랜젝션이 롤백 되어도 외부 트랜젝션은 커밋할 수 있음
    * 외부 트랜젝션이 롤백 되면 중첩 트랜젝션도 함께 롤백 됨
  * 참고
    * JDBC savepoint 기능을 사용함. DB 드라이버에서 해당 기능을 지원하는지 확인 필요
    * 중첩 트랜젝션은 JPA 에서 사용 불가

* 트랜젝션 전파와 옵션
  * isolation , timeout , readOnly 는 트랜젝션이 처음 시작될 때만 적용 됨.
  * 즉, 트랜젝션 참여시에는 적용되지 않음
    * 예를들어 REQUIRED 를 통한 트랜젝션 시작, REQUIRED_NEW 를 통한 트랜젝션 시작 시점에만 적용 됨
